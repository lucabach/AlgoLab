...
void testcase()
{
  ...
  if (s != -1)
  {
    lp.set_a(a_sewer, num_constraints, common_sum_x - noble_sum_x);
  ...
  for (auto point : points)
  {
    ...
    lp.set_a(b_water, num_constraints, y);
  ...
  std::cout << (long) - floor(- CGAL::to_double(final_sol.objective_value()))<< std::endl;
}
